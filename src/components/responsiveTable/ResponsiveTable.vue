<template>
  <div class="wrapper">
    <table name="responsive-table" :color="options.color">
      <thead>
        <tr>
          <th v-for="(col) in colsDef" :key="col.field" :class="{ 'action-column': col.field === 'actions' }">{{ col.header }}</th>
        </tr>
      </thead>
      <tbody>
...
const setActionColumnWidth = () => {
  ...
  if (rootEl) {
    ...
    const buttonsWidth: number = actionColumn[1] ? (Array.from(actionColumn[1].querySelectorAll('button')).reduce((acc: any, button: any) => acc + (button as HTMLElement).offsetWidth, 0) as number) : 0
    if (actionColumn) {
...
<style lang="scss" scoped src="./responsiveTable.scss">
